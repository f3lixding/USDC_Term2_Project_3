...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
	...
	//creating num_particle number of particle class and initializing each class member:
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
	...
	//getting standard deviation from input:
	double std_x = std_pos[0];
	double std_y = std_pos[1];
	double std_theta = std_pos[2];

	//using the std to create normal distribution:
...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
	...
	// to account for number of observation and number of predictions
	unsigned int nObs = observations.size();
	unsigned int nPre = predicted.size();

	// looping through each observation to assign them to the nearest id
	for (unsigned int i = 0; i < nObs; i++) {
		double minDistance = numeric_limits<double>::max();

		int mapID = -1;
		
		// looping through each prediction in each loop of observation to see if they are the closest
